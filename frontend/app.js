/**
 * BuildIntel - Frontend Application
 * AI ассистент для анализа маркетинга в строительстве
 */
        ...
        return `
            ...
            <div class="result-block">
                <h3>
                    ...
                    Общая оценка удобства планировки
                </h3>
            ...
            ${this.renderResultBlock('Анализ удобства планировки', analysis.marketing_insights, 'insights')}
            ${this.renderResultBlock('Сильные и слабые стороны, рекомендации', analysis.recommendations, 'recommendations')}
        `;
    },
    ...
    renderParsedContent(data) {
        ...
        return `
            ${parsed.screenshot_base64 ? `
                <div class="result-block">
                    ...
                    <div style="text-align: center; margin-top: 16px;">
                        <img src="data:image/png;base64,${parsed.screenshot_base64}" 
                             alt="Screenshot" 
            ...
            <div class="parsed-content">
                ...
                ${parsed.full_text && parsed.full_text.length > 0 ? `
                    <div class="label">Весь текст (первые 500 символов):</div>
                    <div class="value">${parsed.full_text.length > 500 ? parsed.full_text.substring(0, 500) + '...' : parsed.full_text}</div>
                ` : ''}
            </div>
...
const handlers = {
    ...
    async handleAnalyzeText() {
        ...
        if (text.length < 10) {
            ui.showError('Введите продающий текст минимум 10 символов для анализа');
            return;
        }
    ...
    async handleAnalyzeImage() {
        if (!state.selectedImage) {
            ui.showError('Выберите планировку квартиры для анализа');
            return;
        }
        ...
        try {
            ...
            if (result.success && result.analysis) {
                ...
                ui.showError(result.error || 'Произошла ошибка при анализе планировки');
            }
        } catch (error) {
            ui.showError('Ошибка соединения с сервером');
            console.error(error);
...
console.log('BuildIntel app loaded');
if (!elements.loadingOverlay) {
